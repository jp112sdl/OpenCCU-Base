...
std::string HS485CommMessageDecoder::ToString(HS485CommMessage* msg)
{
	...
	switch(msg->GetCommand()){
		...
		{
			sprintf(buffer, "Discovery #%d: ", msg->GetID()&0xff);
			result=buffer;
		...
		{
			sprintf(buffer, "Discovery result #%d: ", msg->GetID()&0xff);
			result=buffer;
			result+=Format(*msg, 0, false);
			return result;
		}
		...
		{
			sprintf(buffer, "Error #%d: ", msg->GetID()&0xff);
			result=buffer;
			result+=Format(*msg, 0, false);
			return result;
		}
		...
		case HS485CommMessage::CMD_SEND:
				sprintf(buffer, "%s #%d %08X -> %08X: ", CtrlToString(msg->GetCtrl()), msg->GetID(), (int)msg->GetSenderAddress(), (int)msg->GetReceiverAddress());
				result=buffer;
				result+=Format(*msg, msg->MapIndex(9), true);
				break;
		case HS485CommMessage::CMD_BOOT_SEND:
				sprintf(buffer, "Bootloader %s #%d -> %08X: ", CtrlToString(msg->GetCtrl()), msg->GetID(), (int)msg->GetReceiverAddress());
				result=buffer;
				result+=Format(*msg, msg->MapIndex(9), true);
				break;
		case HS485CommMessage::CMD_RESPONSE:
		{
			sprintf(buffer, "Response %s on #%d : ", CtrlToString(msg->GetCtrl()), msg->GetID());
			result=buffer;
	...
	sprintf(buffer, "#%d Undecoded frame cmd=0x%02X:\n", msg->GetID(), msg->GetCommand());
	return buffer;
}
