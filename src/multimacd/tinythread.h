...
/*
This copy of TinyThread++ has been altered by ise GmbH.
Some bugs were fixed and some features added.
*/
...
#ifdef WIN32
#ifdef TINYTHREAD_BUILD_DLL
#define TINYTHREAD_DLLEXPORT __declspec(dllexport)
#pragma warning( disable: 4251 )
#else
#ifdef TINYTHREAD_BUILD_LIB
#define TINYTHREAD_DLLEXPORT
#else
#define TINYTHREAD_DLLEXPORT __declspec(dllimport)
#pragma warning( disable: 4251 )
#endif
#endif
#else
...
namespace tthread {
	...
	/// on that object).
	/// @see recursive_mutex
	class TINYTHREAD_DLLEXPORT mutex {
	public:
		/// Constructor.
		mutex()
#if defined(_TTHREAD_WIN32_)
	...
	/// number of times).
	/// @see mutex
	class TINYTHREAD_DLLEXPORT recursive_mutex {
	public:
		/// Constructor.
		recursive_mutex()
		{
	...
	/// }
	/// @endcode
	class TINYTHREAD_DLLEXPORT condition_variable {
	public:
		/// Constructor.
#if defined(_TTHREAD_WIN32_)
		condition_variable();
		...
		condition_variable()
		{
			...
			pthread_cond_init(&mHandle, &condattr);
		...
		template <class _mutexT, class _Rep, class _Period> cv_status wait_for(_mutexT &aMutex, const chrono::duration<_Rep, _Period>& aTime)
		{
#if defined(_TTHREAD_WIN32_)
			// Increment number of waiters
			EnterCriticalSection(&mWaitersCountLock);
			...
			// Release the mutex while waiting for the condition (will decrease
			// the number of waiters when done)...
			int milliseconds = int(double(aTime.count()) * (1000.0 * _Period::_as_double()) + 0.5);
			aMutex.unlock();
			cv_status status = _wait( milliseconds );
			aMutex.lock();
			return status;
#else
			...
			int microseconds = int(double(aTime.count()) * (1000000.0 * _Period::_as_double()) + 0.5);
			end_time.tv_sec += microseconds / 1000000;
			microseconds %= 1000000;
			end_time.tv_nsec += (microseconds % 1000000) * 1000;
			if( end_time.tv_nsec >= 1000000000 )
			{
				end_time.tv_sec++;
				end_time.tv_nsec -= 1000000000;
			}
			if( pthread_cond_timedwait(&mHandle, &aMutex.mHandle, &end_time) != 0 )return timeout;
			else return no_timeout;
#endif
		}
...
#if defined(_TTHREAD_WIN32_)
		void _wait();
		cv_status _wait(DWORD milliseconds);
	...
	/// Thread class.
	class TINYTHREAD_DLLEXPORT thread {
	public:
#if defined(_TTHREAD_WIN32_)
		...
		/// (i.e. non-joinable).
		thread() : mHandle(0), mNotAThread(true), mStartInfo(0)
#if defined(_TTHREAD_WIN32_)
			, mWin32ThreadID(0)
#endif
		{}
		...
		/// Move the context information from one \c thread object 
		/// to another one.
		/// @param[in] other The \c thread object to move into this object
	...
	/// @see thread::get_id()
	class TINYTHREAD_DLLEXPORT thread::id {
	public:
		/// Default constructor.
	...
	/// The static class \c this_thread provides methods for dealing with the
	/// calling thread.
	class TINYTHREAD_DLLEXPORT this_thread {
	public:
		...
		static void sleep_for(const chrono::duration<_Rep, _Period>& aTime)
		{
#if defined(_TTHREAD_WIN32_)
